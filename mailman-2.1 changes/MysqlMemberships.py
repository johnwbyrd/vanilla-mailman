...
Massaged with a crowbar by John Byrd <jbyrd@giganticsoftware.com>, 2013/05/21
...
class MysqlMemberships(MemberAdaptor.MemberAdaptor):
    ...
    def __init__(self, mlist):
        ...
        if mm_cfg.MYSQL_MEMBER_DB_VERBOSE:
            ...
            message = "MysqlMemberships " \
                + "$Revision: " + T_REVISION + " initialized with host: %s (%s)" % (
                mm_cfg.connection.get_host_info(),
                mm_cfg.connection.get_server_info() )
		# This can fill up the log too fast -- jwb 2013.02.01
    ...
    def __del__(self):
        ...
        if mm_cfg.MYSQL_MEMBER_DB_VERBOSE:
            ...
            syslog("error", "MysqlMemberships $Revision: " + T_REVISION + " unloaded" )
            syslog("mysql", "MysqlMemberships $Revision: " + T_REVISION + " unloaded" )

    # Find out whether we should be using 'flat' or 'wide' table type.
    # We use a default of 'flat' for working with Vanilla.
    def getTableType(me):
    ...
    def createTable(self):
		# This query throws a warning anyway if the table exists; need to silence
		# that to avoid messages to stdout
